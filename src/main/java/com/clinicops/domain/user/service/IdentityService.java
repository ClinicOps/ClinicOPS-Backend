...
public class IdentityService {
	...
	/**
	 * Enhanced register flow: 1. Validate input 2. Create/find Organization 3.
	 * Create Clinic under Organization 4. Create User 5. Assign OWNER role to user
	 * in clinic (UserRoleAssignment) 6. Create ClinicMember entry 7. Generate
	 * tokens 8. Return AuthResponse with user details
	 */
	@Transactional
	public AuthResponse register(RegisterRequest request) {
		...
		String accessToken = jwtService.generateAccessToken(user.getId(), organization.getId(), clinic.getId(),
				"OWNER");
		...
		refreshTokenEntity.setTokenHash(org.apache.commons.codec.digest.DigestUtils.sha256Hex(refreshToken));
		...
		UserDTO userDTO = new UserDTO(user.getId().toString(), user.getEmail(), organization.getId().toString(),
				clinic.getId().toString(), clinic.getName(), clinic.getTimezone(), "OWNER");
	...
	public AuthResponse login(LoginRequest request) {
		...
		List<ClinicMember> activeMemberships = clinicMemberRepository.findByUserIdAndStatusAndDeletedFalse(user.getId(),
				MembershipStatus.ACTIVE);

		List<ClinicMember> allMemberships = clinicMemberRepository.findByUserIdAndDeletedFalse(user.getId());

		if (!allMemberships.isEmpty() && activeMemberships.isEmpty()) {
			throw new BusinessException("No active clinic access");
		}
		...
		if (!activeMemberships.isEmpty()) {
			...
			ClinicMember m = activeMemberships.get(0);
			...
			if (clinic.getStatus() != ClinicStatus.ACTIVE) {
				throw new BusinessException("Clinic is inactive");
			}
			...
			clinicTimezone = clinic.getTimezone();
		}

		String accessToken = jwtService.generateAccessToken(user.getId(), orgId, clinicId, role);

		String refreshToken = jwtService.generateRefreshToken(user.getId());

		RefreshToken entity = new RefreshToken();
		entity.setUserId(user.getId());
		entity.setTokenHash(org.apache.commons.codec.digest.DigestUtils.sha256Hex(refreshToken));
		entity.setExpiryDate(java.time.Instant.now().plusSeconds(604800));
		entity.setRevoked(false);

		refreshTokenRepository.save(entity);

		// Build UserDTO for response
		UserDTO userDTO = new UserDTO(user.getId().toString(), user.getEmail(), orgId != null ? orgId.toString() : null,
				clinicId != null ? clinicId.toString() : null, clinicName, clinicTimezone, role);
	...
	private void publishRegistrationAudit(ObjectId userId, ObjectId clinicId, ObjectId organizationId, String email) {
		try {
			...
			AuditRecord auditRecord = AuditRecord.builder().userId(userId).clinicId(clinicId).domain("access")
					.resource("user").action("register").timestamp(java.time.Instant.now()).metadata(metadata).build();
	...
	public AuthResponse refreshToken(RefreshRequest request) throws AuthorizationException {
		...
		String tokenHash = org.apache.commons.codec.digest.DigestUtils.sha256Hex(refreshToken);

		RefreshToken stored = refreshTokenRepository.findByTokenHashAndRevokedFalse(tokenHash)
				.orElseThrow(() -> new AuthorizationException("Refresh token invalid"));

		if (stored.getExpiryDate().isBefore(Instant.now())) {
			throw new AuthorizationException("Refresh token expired");
		}

		User user = userRepository.findById(userId).orElseThrow(() -> new AuthorizationException("User not found"));

		// 🔒 Re-validate memberships
		List<ClinicMember> activeMemberships = clinicMemberRepository.findByUserIdAndStatusAndDeletedFalse(userId,
				MembershipStatus.ACTIVE);

		List<ClinicMember> allMemberships = clinicMemberRepository.findByUserIdAndDeletedFalse(userId);

		if (!allMemberships.isEmpty() && activeMemberships.isEmpty()) {
			throw new AuthorizationException("No active clinic access");
		}
		...
		if (!activeMemberships.isEmpty()) {
			...
			Clinic clinic = clinicRepository.findById(clinicId)
					.orElseThrow(() -> new AuthorizationException("Clinic not found"));

			if (clinic.getStatus() != ClinicStatus.ACTIVE) {
				throw new AuthorizationException("Clinic inactive");
			}

			Organization org = organizationRepository.findById(orgId)
					.orElseThrow(() -> new AuthorizationException("Organization not found"));

			if (org.getStatus() != OrganizationStatus.ACTIVE) {
				throw new AuthorizationException("Organization inactive");
			}
		...
		String newAccessToken = jwtService.generateAccessToken(userId, orgId, clinicId, role);
